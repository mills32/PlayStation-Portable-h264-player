...
SET S6=%S5%:b_bias=0:direct="spatial":weightb=1:open_gop=0:weightp=2:keyint=65536:keyint_min=65536
SET S7=%S6%:scenecut=40:intra_refresh=0:rc_lookahead=40:mbtree=1:crf=23.0
SET _OPT=%S7%:qcomp=0.60:qpmin=0:qpmax=69:qpstep=4:ip-factor=1.40:aq_mode=1:aq_strength=1.00:no_info=1
...
echo.   
echo SELECT VIDEO SIZE (only homebrew players; default 480x272):  
echo - 0 480x272;
echo - 1 720x480 (TV OUT PSP 2000+);       
...
if %VIDEO_CONTAINER% equ 0 (echo MP4 CONTAINER: set default 480x272)
if %VIDEO_CONTAINER% equ 0 (set SIZE_FILTERS=-s 480x272 -bsf:v "filter_units=remove_types=6|35|38-40")
...
echo - 3 VERY HIGH (600kbps: CRASH/SLOW WHEN MORE THAN 30FPS, USE 720x480 24FPS);   
...
ffmpeg -i %VIDEO_NAME% -c:v libx264 -profile:v main %SIZE_FILTERS% -x264-params "%_OPT%%BITRATE%" %AUDIO_FORMAT% psp_video.%CONTAINER%
pause